...
<html lang="fr" dir="ltr">
	...
	echo "<div class=\"text-success\"><h2 class=\"font-weight-light\">Modify this Transaction : </h2></div>";
	...
	if (isset($_POST['submit'])){
		...
		if (!empty($message)) {
		mysqli_query($conn, "UPDATE transactions SET message = '$message' WHERE transac_id = '$transac_id'");
		echo' <p style="color:#21c004;">Message Modified successfully</p>';
		}
		
		if (!empty($amount)) {
		mysqli_query($conn, "UPDATE transactions SET amount = '$amount' WHERE transac_id = '$transac_id'");
		echo' <p style="color:#21c004;">Amount Modified successfully</p>';
		}
	...
	<form action="" method="post">
			...
			<label class="sr-only">Modified Message</label>
			<div class="col-sm-10 form-group text-left">
				<input type="text" name="explicatif_message" class="form-control" placeholder="Modified Message">
			</div>
			</div> </div> </div>


			<div class="d-flex justify-content-center"> <div class="w-50 p-1"> <div class="d-flex justify-content-center mx-5">
			<label class="sr-only">Modified Amount</label>
			<div class="col-sm-10 form-group text-left">
				<input type="text" name="amount" class="form-control" placeholder="Modified Amount (‎€)">
			</div>
			</div> </div> </div>
