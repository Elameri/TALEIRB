<?php
	...
	{
		$receiver_pseudo = $_POST['receiver_pseudo'];
...
<html lang="fr" dir="ltr">
	...
	<!-- début du formulaire -->
	<div class="text-primary"><h2 class="font-weight-light">I'm the sender of this Transaction</h2></div>
	...
	if (isset($_POST['submit'])){
		...
		$result = mysqli_query($conn, "SELECT id FROM users WHERE email='$receiver_email'");
		$row = mysqli_fetch_array($result);
		$receiver_id = $row['id'];
		}
			
		if (!empty($receiver_pseudo)) {
		//à partir du pseudo: récuperer l'id
		$result = mysqli_query($conn, "SELECT id FROM users WHERE pseudo='$receiver_pseudo'");
		$row = mysqli_fetch_array($result);
		$receiver_id = $row['id'];
		}
		
		if (!empty($receiver_id)) {
			$est_ce_un_ami = mysqli_query($conn, " SELECT id FROM friends WHERE (id='$notre_id' AND id_ami='$receiver_id') OR (id='$receiver_id' AND id_ami='$notre_id') ");
			...
			if($num == 0){
				...
				mysqli_query($conn, " insert into transactions(sender_id, receiver_id, message, amount, date, status) values ('$notre_id', '$receiver_id', '$message', '$amount', '$date', '$status')");
	...
	<form action="" method="post">
			...
			<label class="sr-only">The Receiver</label>
			<div class="col-sm-10 form-group text-left">
				<input type="text" name="receiver_pseudo" class="form-control" placeholder="Receiver's @Pseudo">
			</div>
			</div> </div> </div>
			...
			<label class="sr-only">The Receiver</label>
			<div class="col-sm-10 form-group text-left">
				<input type="email" name="receiver_email" class="form-control" placeholder="OR Receiver's email @address">
			</div>
			</div> </div> </div>
